...
**`order_number` remains unmasked; `purchase_order_number` and `warehouse_code` are now pseudonymized.** `order_number` retains analytical value for joins, duplicate investigation, and operational tracking, and is not treated as a direct identifier in this environment. `purchase_order_number` carried residual risk because a customer can embed identifying information in its own numbering convention, so it is now replaced with a same-length deterministic pseudonym (`deterministic_pseudonym()` in `src/privacy/hash_utils.py`) rather than left unmasked — same for inventory's `warehouse_code`. Unlike customer/product pseudonymization, these use a direct salted-hash derivation instead of a persisted mapping table: nothing downstream needs a human-readable value back, only a stable one, so a mapping table would add state for no benefit. The output is deterministic (same real value -> same pseudonym, given a fixed salt) and the same length as the input, rather than a fixed 64-char hash — so composite keys that include these fields (e.g. `order_number + purchase_order_number + full_sku_code` in the Snowflake RAW dedup checks) keep working unchanged. Tradeoff: matching the exact input length means short values (`purchase_order_number` runs 1-12 characters in the observed source) have real collision risk — a length-1 value has only 36 possible outputs. `purchase_order_number` isn't part of the actual adopted business key (`company_code + order_number + unique_sku_code`), so this is an accepted accuracy loss for operational tracking, not a broken join. `hash_utils.py` also keeps `deterministic_hash()`, the original fixed-64-char version, for any future field where collision resistance matters more than length matching.

**Employee usernames are dropped; `company_code` is retained, now pseudonymized.** `created_by_user` is a direct employee identifier and is removed before the final order output. `company_code` stays present — it is required by the output/quality-gate contract and is part of the Snowflake RAW dedup composite key (`company_code + order_number + unique_sku_code`) — but as of the `purchase_order_number`/`warehouse_code` pseudonymization change, its literal value is replaced with the same same-length deterministic pseudonym before upload, so it's no longer carried through unmasked. An earlier design note said both `created_by_user` and `company_code` were dropped; that no longer describes the deployed code, and full deletion of `company_code` was reconsidered again here and rejected for the same reason: it would break `REQUIRED_COLUMNS` in both quality gates and the Snowflake dedup key with no equivalent replacement.

**`delivery_route_name` is pseudonymized with a mapped identity; `delivery_route` is left unmasked.** Open orders' `delivery_route_name` is free text describing a physical route, and in practice it routinely embeds a real customer name and city/province (e.g. `"COSTCO - AIRDRIE (AB)"`) — discovered only after `purchase_order_number`/`company_code`/`warehouse_code` were already handled, and a more direct identity leak than any of those, since it was never in `HASH_COLUMNS` or `REQUIRED_ORDER_COLUMNS` at all. It gets the same treatment as `customer_name` — a stable, human-readable pseudo identity (`"Route RT-000042"`) from a persisted local mapping (`route_name_mapping_final.csv`, built by `route_name_mapping_pipeline.py`) — rather than the same-length scramble used for `purchase_order_number`/`company_code`/`warehouse_code`, because a mapped identity is what "meaningful, not gibberish" requires for a field this legible. Order history has no equivalent field (only the numeric `delivery_route` code), so this mapping doesn't need the shared-foundation/backfill machinery customer and product identity use — it's self-contained to the open_orders track. `delivery_route` itself stays unmasked, by explicit choice: it's a low-cardinality operational code, not free text, and doesn't carry the same embedded-identity risk.
